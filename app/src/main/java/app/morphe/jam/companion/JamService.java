...
public final class JamService extends Service {
    ...
    private final Object serial=new Object(), connectionLock=new Object();
    private final Map<String,String> completed=new HashMap<>(), payloads=new HashMap<>();
    ...
    private volatile String role="Idle",transport="None",message="Open Jam in YouTube Music to pair this layer.";
    ...
    private final ServiceConnection binding=new ServiceConnection(){
        public void onServiceConnected(ComponentName n,IBinder b){bridge=IJamBridge.Stub.asInterface(b);message="YouTube Music connected";}
        public void onServiceDisconnected(ComponentName n){bridge=null;message="Waiting for YouTube Music";}
        public void onBindingDied(ComponentName n){bridge=null;if(bound){unbindService(this);bound=false;}new Handler(getMainLooper()).postDelayed(()->bindMusic(),1000);}
    };
    private android.content.SharedPreferences prefs(){return getSharedPreferences("pair",0);}
    ...
    @Override public void onCreate(){super.onCreate();active=this;bindMusic();
        wakeLock=getSystemService(PowerManager.class).newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,"MorpheJam:session");
    ...
    private void serve(Socket socket,Invitation session){
        try(SecureChannel peer=new SecureChannel(socket,true,session.jamId,session.secret,null)){
            android.util.Log.i("MorpheJam","Authenticated participant");message="Participant authenticated";
            while(invitation==session && session.valid()){
                String request=peer.receive();
                JSONObject command=new JSONObject(request);String op=command.optString("op");
                if("PING".equals(op)){peer.send(ok().toString());continue;}
                if("SYNC".equals(op)){
                    String reply;
                    synchronized(serial){
        ...
        finally{sockets.remove(socket);try{socket.close();}catch(Exception ignored){}}
    }
    private JSONObject hostCall(JSONObject request,String client)throws Exception{
        synchronized(serial){
            ...
            if(payloads.containsKey(key)){if(!payloads.get(key).equals(body))return error("Command id reused");return new JSONObject(completed.get(key));}
            if(payloads.size()>=4096)return error("Session command limit reached; start a new session");
            payloads.put(key,body);JSONObject response;
            try{response=music(request);}catch(Exception e){response=error("Outcome unknown; refresh queue before another edit");}
            if(response.optBoolean("ok")&&response.has("items"))sharedQueue=response;
            completed.put(key,response.toString());return response;
        }
    }
    ...
    public JSONObject dispatch(JSONObject request)throws Exception{
        ...
        if("INVITE".equals(request.optString("op"))){
            String value=invite();if(value.isEmpty())return error("Only the host can invite");
            com.google.zxing.common.BitMatrix bits=new com.google.zxing.MultiFormatWriter().encode(value,com.google.zxing.BarcodeFormat.QR_CODE,600,600);
            android.graphics.Bitmap bitmap=android.graphics.Bitmap.createBitmap(600,600,android.graphics.Bitmap.Config.ARGB_8888);
    ...
    public void end(){
        sessionEpoch++;if(codePairing!=null){codePairing.close();codePairing=null;}
        Invitation old=invitation;invitation=null;role="Idle";transport="None";sharedQueue=null;allowGuestEdits=true;
        if(nearby!=null){nearby.close();nearby=null;}try{if(server!=null)server.close();}catch(Exception ignored){}server=null;
        for(Socket s:sockets)try{s.close();}catch(Exception ignored){}sockets.clear();
        synchronized(connectionLock){if(channel!=null)try{channel.close();}catch(Exception ignored){}channel=null;}
        synchronized(serial){completed.clear();payloads.clear();}if(old!=null)old.destroy();
        if(wakeLock!=null&&wakeLock.isHeld())wakeLock.release();
    }
    @Override public void onDestroy(){end();if(bound)unbindService(binding);workers.shutdownNow();monitor.shutdownNow();active=null;super.onDestroy();}
}
