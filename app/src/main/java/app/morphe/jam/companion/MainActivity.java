...
public final class MainActivity extends Activity {
  ...
  private final Runnable refresh = new Runnable() {
    public void run() {
      if (status != null) {
        JamService s = JamService.active;
        if (s == null) status.setText(
          "Ready to connect\nStart or join a Jam from YouTube Music's player."
        );
        else {
          org.json.JSONObject state = s.state();
          status.setText(
            state.optString("role") +
              " · " +
              state.optString("transport") +
              "\n" +
  ...
  public void onCreate(Bundle b) {
    ...
    if ("app.morphe.jam.SCAN".equals(getIntent().getAction())) {
      ...
      try {
        if (
          caller == null || !caller.equals(p.getString("package", ""))
        ) throw new SecurityException();
        Trust.capability(
          p.getString("cap", null),
          getIntent().getStringExtra("cap")
        );
        scanning = true;
      } catch (Exception e) {
        finish();
    ...
    title.setText("Jam Layer");
    title.setTextSize(28);
    body.addView(title);
    TextView description = new TextView(this);
    description.setText(
      "Nearby connections for YouTube Music.\n\nYour queue, invitations and session controls are in the YouTube Music player. This layer keeps the devices connected."
    );
    ...
    body.addView(status);
    button(body, "Open YouTube Music", () -> {
      String pkg = getSharedPreferences("pair", 0).getString(
        "package",
        "app.morphe.jam.next.music"
      );
      Intent launch = getPackageManager().getLaunchIntentForPackage(pkg);
      if (launch != null) startActivity(launch);
      else Toast.makeText(
        this,
        "Install the Jam-enabled YouTube Music build",
        Toast.LENGTH_LONG
      ).show();
    });
    button(body, "Nearby permissions", this::permissions);
    button(body, "Disconnect", () -> {
  ...
  private void pair() {
    ...
    if (!validPackage(pkg) || cap == null || cap.length() != 64) {
      finish();
      return;
    }
    new AlertDialog.Builder(this)
      ...
      .setMessage(
        "Allow " +
          pkg +
          " to use Jam Layer for nearby queue sharing. This approval is bound to the selected package and a local capability token."
      )
      ...
      .setPositiveButton("Connect", (d, w) -> {
        ...
        setResult(RESULT_OK);
        finishAfterPermissions = true;
        if (!permissions()) finish();
      })
  ...
  private boolean permissions() {
    ...
    } else permissions.add("android.permission.ACCESS_FINE_LOCATION");
    if (Build.VERSION.SDK_INT >= 37) permissions.add(
      "android.permission.ACCESS_LOCAL_NETWORK"
    );
    permissions.removeIf(
      p -> checkSelfPermission(p) == PackageManager.PERMISSION_GRANTED
    );
    if (!permissions.isEmpty()) {
      requestPermissions(permissions.toArray(new String[0]), 2);
      return true;
    }
    return false;
  }
  ...
  @Override
  public void onRequestPermissionsResult(int code, String[] p, int[] grants) {
    super.onRequestPermissionsResult(code, p, grants);
    if (code == 2 && finishAfterPermissions) {
      finish();
      return;
    }
    if (code == 3) {
  ...
  private void joinWhenReady(String invite, int attempts) {
    JamService s = JamService.active;
    if (s != null) {
      s.join(invite, "Auto");
      setResult(RESULT_OK);
      finish();
    } else if (attempts > 0) handler.postDelayed(
      () -> joinWhenReady(invite, attempts - 1),
      100
    );
    else finish();
  }
