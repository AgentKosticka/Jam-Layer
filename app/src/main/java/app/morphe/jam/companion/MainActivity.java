...
public final class MainActivity extends Activity {
    ...
    private void pair(){
        ...
        if(!Arrays.asList("app.morphe.jam.probe.music","app.morphe.android.apps.youtube.music","com.google.android.apps.youtube.music").contains(pkg)||cap==null||cap.length()!=64){finish();return;}
        String cert=Trust.certificate(this,pkg);
        new AlertDialog.Builder(this).setTitle("Connect YouTube Music?").setMessage("Allow this YouTube Music installation to use Jam Layer for nearby queue sharing.")
            .setNegativeButton("Cancel",(d,w)->finish()).setPositiveButton("Connect",(d,w)->{
                if(JamService.active!=null)JamService.active.end();
                getSharedPreferences("pair",0).edit().putString("package",pkg).putString("cert",cert).putString("cap",cap).commit();setResult(RESULT_OK);finish();
    ...
    private void permissions(){
        ...
        permissions.removeIf(p->checkSelfPermission(p)==PackageManager.PERMISSION_GRANTED);if(!permissions.isEmpty())requestPermissions(permissions.toArray(new String[0]),2);
    }
    private void scan(){startActivityForResult(new Intent(this,ScanActivity.class),4);}
    @Override public void onRequestPermissionsResult(int code,String[] p,int[] grants){super.onRequestPermissionsResult(code,p,grants);if(code==3){if(grants.length>0&&grants[0]==PackageManager.PERMISSION_GRANTED)scan();else finish();}}
